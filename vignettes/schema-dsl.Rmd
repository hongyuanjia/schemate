...
24. Each `groups[]` item must contain `names` plus exactly one primary operator.
25. `groups[]` items must not wrap the target node inside an anonymous nested
    list.
26. `version` is only allowed at the root schema document.
27. `version` must be a non-empty string when present.
28. `$defs` is only allowed at the root schema document.
29. `$defs` must be an object whose values are complete schema nodes.
30. `$ref` must be a local string reference of the form `#/$defs/name`.
31. A `$ref` target must exist in the current document's `$defs`.
32. A `$ref` node must not contain other schema keys except `description`.
33. `description` must be a non-empty string when present.
