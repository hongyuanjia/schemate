...
6. Container kinds are:
   ...
   - `data_table`
   - `tibble`
7. `fields`, `groups`, and `keys` are only valid on `check` nodes whose
   `check.kind` is a container kind.
8. `fields["*"]` is the wildcard child schema for otherwise unspecified keys.
9. `groups` can batch-assign multiple field names to one shared child schema
   node.
10. A top-level schema document may optionally declare `version` as a
    non-empty string representing the document version.
11. Root-level `$defs` can store reusable schema nodes and `$ref` can reuse them.
12. Inside `all` / `any` / `one` / `not`, a branch may use shorthand check
...
The reserved field name `"*"` is the wildcard child schema.
...
This means that any otherwise unspecified field must satisfy the wildcard schema.

### Closed schema rule

If a container-check node has `fields` but no `fields["*"]`, then the node is treated
as a closed schema: unspecified fields are not allowed.

### Precedence rule

If both explicit fields and `fields["*"]` are present:

- explicit fields use their own schema
- all other keys use the wildcard schema
...
- group names must not overlap with explicit `fields` (except `fields["*"]`)
...
3. if `check.kind` is a container kind, recursively validate child fields
