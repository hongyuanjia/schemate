...
When the same edit should apply to several schema nodes, find the logical paths
first, then replace the matching nodes. Logical paths expand grouped fields into
...
Use validation modes according to the caller. The examples below use one small
schema so the return shape is easy to compare.
...
`assert` is the default for application code. It returns the input invisibly on
...
`check` returns `TRUE` or a diagnostic string. It is useful when you want to show
or store the validation message.
